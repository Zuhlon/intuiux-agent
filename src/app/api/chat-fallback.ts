...
function generateContextualCJM(idea: Idea, _analysis: SourceAnalysis): {
  ...
  // Build stages based on product type
  const stages = [
    {
      name: 'Осознание потребности',
      actions: ['Поиск информации', 'Чтение статей', 'Обсуждение с коллегами'],
      emotions: ['Интерес', 'Любопытство'],
      painPoints: ['Много информации', 'Сложно найти качественный контент'],
      opportunities: ['SEO-оптимизация', 'Контент-маркетинг']
    },
    {
      name: 'Поиск решения',
      actions: ['Сравнение платформ', 'Чтение отзывов', 'Тестирование'],
      emotions: ['Ожидание', 'Скепсис'],
      painPoints: ['Сложно сравнить', 'Не понятно что подходит'],
      opportunities: ['Сравнительные таблицы', 'Демо-контент']
    },
    {
      name: 'Принятие решения',
      actions: ['Регистрация', 'Выбор тарифа', 'Оплата'],
      emotions: ['Волнение', 'Надежда'],
      painPoints: ['Сложная регистрация', 'Не понятно что получу'],
      opportunities: ['Простой onboarding', 'Бесплатный период']
    },
    {
      name: 'Использование',
      actions: ['Чтение контента', 'Использование инструментов', 'Участие в комьюнити'],
      emotions: ['Удовлетворение', 'Вовлечённость'],
      painPoints: ['Не нахожу нужное', 'Мало интерактива'],
      opportunities: ['Персонализация', 'Умные рекомендации']
    },
    {
      name: 'Лояльность',
      actions: ['Подписка', 'Рекомендация', 'Участие'],
      emotions: ['Доверие', 'Принадлежность'],
      painPoints: ['Забываю заходить', 'Нет ощущения сообщества'],
      opportunities: ['Push-уведомления', 'Программа лояльности']
    }
  ];
  
  // Build mermaid journey
  const mermaidSections = stages.map((stage, i) => {
    const sectionActions = stage.actions.map((a, j) => `      ${a}: ${i * 3 + j + 1}`).join('\n');
...
| Аспект | Описание |
|--------|----------|
| **Действия** | ${stage.actions.join(', ')} |
| **Эмоции** | ${stage.emotions.join(', ')} |
| **Боли** | ${stage.painPoints.join(', ')} |
| **Возможности** | ${stage.opportunities.join(', ')} |`).join('\n\n');
  
  const recommendations = stages.flatMap(s => s.opportunities).slice(0, 5).map((r, i) => `${i + 1}. ${r}`).join('\n');
  
  return {
    title: `Путь пользователя "${productName}"`,
    mermaidSections,
    detailedTable,
    recommendations
  };
...
function extractSourceText(message: string): string {
  ...
  const dialogLines = message.split('\n').filter(line => /^[A-ZА-ЯЁ][a-zA-Za-яё]+:\s/.test(line.trim()));
