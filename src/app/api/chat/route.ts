...
// v3.0: Use new idea-extractor module to fix matchAll issues
...
Отвечай на русском языке.`,
  ...
  cjm_researcher: `Ты — Senior UX Researcher с 15-летним опытом создания Customer Journey Maps.

═══════════════════════════════════════════════════════════════
📚 ПРОФЕССИОНАЛЬНАЯ МЕТОДОЛОГИЯ CJM
═══════════════════════════════════════════════════════════════

Основано на 5 источниках:
...
- Персона должна соответствовать целевой аудитории из идеи
- Touchpoints должны отражать КОНКРЕТНЫЕ функции продукта
- Эмоции должны иметь триггеры (что вызвало эту эмоцию)
- Боли должны быть конкретными, не абстрактными
- Возможности должны быть actionable (можно выполнить)

❌ ЗАПРЕЩЕНО:
- Общие фразы типа "пользователь ищет информацию"
- Выдуманные данные, которых нет в идее
- Шаблонные боли типа "сложность выбора"
- Абстрактные рекомендации без привязки к продукту

═══════════════════════════════════════════════════════════════
📝 ОБЯЗАТЕЛЬНАЯ СТРУКТУРА CJM
═══════════════════════════════════════════════════════════════

## 🗺️ Customer Journey Map: [Название продукта]

### Персона: [Имя] — [Роль]
- **Демография:** [возраст, профессия, контекст]
- **Цели:** [2-3 КОНКРЕТНЫЕ цели для этого продукта]
- **Фрустрации:** [2-3 КОНКРЕТНЫЕ боли для этой аудитории]
- **Мотивации:** [почему выбирает этот продукт]

### 📊 Mermaid Journey
\`\`\`mermaid
journey
    title Путь [Персона]: [Название продукта]
    section Осознание потребности
      [Конкретное действие 1]: 1
      [Конкретное действие 2]: 2
    section Поиск и исследование
      [Изучает функции продукта]: 3
    ...
\`\`\`

### Этап 1: Осознание потребности
*Описание КОНКРЕТНО для этого продукта*

| Аспект | Детали |
|--------|--------|
| **Действия** | [Что КОНКРЕТНО делает с упоминанием продукта] |
| **Мысли** | "[Прямая цитата с упоминанием продукта/функции]" |
| **Эмоция** | [название] ([-5 до +5]) — [триггер] |
| **Touchpoints** | [Точки контакта] |
| **Боли** | [КОНКРЕТНЫЕ боли для этого продукта] |
| **Возможности** | [Actionable рекомендации] |

[Повторить для каждого этапа]

### 📈 Эмоциональная дуга
- **Тренд:** улучшение/ухудшение/смешанный
- **Низшая точка:** [этап и причина]
- **Высшая точка:** [этап и причина]

### ⚡ Моменты истины
1. [Критический момент] → [как улучшить]
2. [Критический момент] → [как улучшить]

### 💡 Quick Wins (приоритетные рекомендации)
1. **[Рекомендация]** — [ожидаемый эффект]
2. **[Рекомендация]** — [ожидаемый эффект]

═══════════════════════════════════════════════════════════════
🎯 ЭТАПЫ CJM (стандартные)
═══════════════════════════════════════════════════════════════

1. Осознание потребности — пользователь понимает проблему
2. Поиск и исследование — изучает решение (ВКЛЮЧАЯ этот продукт)
3. Оценка и сравнение — сравнивает с конкурентами
4. Принятие решения — регистрируется/покупает
5. Использование — активно использует функции продукта
6. Лояльность — рекомендует, продлевает

ВАЖНО: На каждом этапе упоминай КОНКРЕТНЫЕ функции и название продукта!
...
function generateContextualCJM(idea: {
  ...
  // Генерируем детальную таблицу с цветовой кодировкой ролей
  const tableRows = stages.map(stage => {
    ...
    // Добавляем цветовую кодировку ролей
    const coloredGoal = `[:role:persona]${goal}[/role]`;
    const coloredTouchpoints = touchpoints.split(', ').map(t => `[:role:touchpoint]${t}[/role]`).join(', ');
    const coloredEmotion = `[:role:emotion]${avgEmotion}/5[/role]`;
    const coloredPain = `[:role:pain]${pain}[/role]`;

    return `| ${stage.name} | ${coloredGoal} | ${coloredTouchpoints} | ${coloredEmotion} | ${coloredPain} |`;
  }).join('\n');
...
${tableRows}`;
  ...
  // Рекомендации по улучшению с цветовой кодировкой
  ...
  stages.forEach(stage => {
    if (stage.steps.some(s => s.emotion <= 3)) {
      if (stage.name === 'Осознание') {
        recommendations.push('[:role:opportunity]**Осознание:** Усилить SEO и контент-маркетинг, чтобы пользователи находили продукт при поиске[/role]');
      } else if (stage.name === 'Поиск') {
        recommendations.push('[:role:opportunity]**Поиск:** Упростить сравнение с конкурентами, добавить наглядную таблицу преимуществ[/role]');
      } else if (stage.name === 'Регистрация') {
        recommendations.push('[:role:opportunity]**Регистрация:** Минимизировать поля формы, добавить вход через соцсети[/role]');
      }
    }
  });
  ...
  if (idea.functions.length >= 3) {
    recommendations.push(`[:role:persona]**Использование:** Акцент на ключевых функциях: ${idea.functions.slice(0, 2).join(', ')}[/role]`);
  }

  // Рекомендации на основе рисков
  if (idea.risks.length > 0) {
    const mainRisk = idea.risks[0].substring(0, 50);
    recommendations.push(`[:role:pain]**Митигация риска:** ${mainRisk}... — предусмотреть решение на этапе онбординга[/role]`);
  }

  // Рекомендации на основе ценности
  if (idea.valueProposition) {
    recommendations.push(`[:role:emotion]**Ценность:** Подчеркнуть в CJM: "${idea.valueProposition.substring(0, 60)}..."[/role]`);
  }
...
function getFallbackResponse(agentType: string, message: string): string {
  ...
  if (agentType === 'brand_marketer') {
    // ПРИОРИТЕТ: Сначала проверяем отрасль в INDUSTRIES_DATABASE (более точное определение)
    const fullContext = message + ' ' + idea.name + ' ' + idea.description + ' ' + idea.functions.join(' ');
    const detectedIndustry = detectIndustry(fullContext);
    
    // Используем detectProductType только как fallback
...
**Целевая аудитория:**
[:role:persona]${idea.userTypes}[/role]

**Ключевая ценность:** [:role:emotion]${idea.valueProposition}[/role]

**Точки боли:**
${idea.risks.slice(0, 3).map((r, i) => `[:role:pain]${i + 1}. ${r}[/role]`).join('\n')}
